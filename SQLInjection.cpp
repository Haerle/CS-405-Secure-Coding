...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  if (sql.find("or") != std::string::npos) {
      ...
      std::cout << "Detected 'OR' in query! Possible SQL Injection!" << std::end1;
      std::cout << "Stopped Query: " << sql.c_str() << std::end1;
      std::cout << "Escaped/Cleaned version of query: " << cleaned << std::end1;
      return false;
  }
