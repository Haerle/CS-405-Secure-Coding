#include "pch.h"// Uncomment the next line to use precompiled headers
...
TEST_F(CollectionTest, CanAddFiveValuesToVector)
{
    ...
    std::cout << "Collection Size: " << (int)collection_size() << '\n';
}

// TODO: Create a test to verify that max size is greater than or equal to size for 0, 1, 5, 10 entries
TEST_F(CollectionTest, IsMaxSizeGreaterThan)
{
...
TEST_F(CollectionTest, IsCapacityGreaterThan)
{
    ASSERT_GE(collection->capacity(), collection->size());
    add_entries(1);
    ASSERT_GE(collection->capacity(), collection->size());
    add_entries(4);
    ASSERT_GE(collection->capactiy(), collection->size());
    add_entries(5);
    ASSERT_GE(collection->capacity(), collection->size());
...
TEST_F(CollectionTest, ResizingIncreaseVector)
{
    ASSERT_TRUE(collection->empty());
...
TEST_F(CollectionTest, ResizingDecreaseVector)
{
    ASSERT_TRUE(collection->empty());
    std::cout << "Collection size: " << (int)collection->size() << '\n';
    add_entries(50);
...
TEST_F(CollectionTest, ResizingDecreaseVectorToZero)
{
    ASSERT_TRUE(collection->empty());
    std::cout << "Collection size: " << (int)collection->size() << '\n';
    add_entries(25);
...
TEST_F(CollectionTest, EraseWorkToEraseContents)
{
    ASSERT_TRUE(collection->empty());
    std::cout << "Collection size: " << (int)collection->size() '\n';
    add_entries(5);
    ...
    for (std::vector<int>::interator it = collection->begin(); it != collection->end(); ++it)
        std::cout << ' ' << *it;
    std::cout << '\n';
    return;
...
TEST_F(CollectionTest, ReserveOnlyChangeCapactiy)
{
    ASSERT_TRUE(collection->empty());
    std::cout << "Collection size: " << (int)collection->size() << '\n';
    std::cout << "Collection capacity: " << (int)collection->capacity() << '\n';
    ...
    ASERT_EQ(collection->capacity(), 55);
